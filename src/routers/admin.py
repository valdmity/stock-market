...
async def add_instrument(instrument: CreateInstrument, authorization: str | None = Depends(header_scheme)):
...
async def user_delete(user_id: str, authorization: str | None = Depends(header_scheme)):
    await check_admin(authorization)
    return await delete_user(user_id)

@admin_router.delete("/instrument/{ticker}", response_model=Result)
async def instrument_delete(ticker: str, authorization: str | None = Depends(header_scheme)):
    await check_admin(authorization)
    await delete_instrument(ticker)
    return Result(success=True)

@admin_router.post("/balance/deposit", response_model=Result, tags=["balance"])
async def balance_deposit(balance: Balance, authorization: str | None = Depends(header_scheme)):
    await check_admin(authorization)
...
async def balance_withdraw(balance: Balance, authorization: str | None = Depends(header_scheme)):
    await check_admin(authorization)
    instrument_id = await get_instrument_id(balance.ticker)
    if not instrument_id:
        raise HTTPException(status_code=400, detail="Instrument not exists")
